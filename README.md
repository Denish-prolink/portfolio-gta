...
Nine original AI-generated backgrounds feature a realistic 3D game protagonist as a software engineer, with GTA-inspired coastal city views and activities matching each section. PNG originals are included in `art-source/`; the site uses optimized WebP versions in `public/art/`. Run `node scripts/optimize-art.mjs` to regenerate the WebP files. See **`PROMPTS.md`** for the generation prompts.
